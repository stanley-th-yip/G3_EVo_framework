...
def open_earlyexit(sys, gas, melt): 
    wt.writeout_crash(sys, gas, melt, np.arange(float(sys.run.P_START), float(sys.P), float(sys.run.DP_MAX)*-1.0))
...
def runtime_error(sys, gas, melt, msg):
    if sys.run.RUN_TYPE == 'open':
        ...
        # PL: need to check if a run has completed - if this crashes in the iron_step it won't work because half the stuff hasn't been computed.
        closed_earlyexit(sys, gas, melt)
...
def scss_warn(scss, s, sys, gas, melt):
    ...
    exit(f'You have reached sulphide saturation, this code is no longer valid. SCSS: {scss:.1} ppm, Sulfide in melt: {s:.1} ppm\nRun is terminated here, results up to this point have been exported to CSV.')
