...
def sat_pressure(run, sys, gas, melt, mols):
    ...
    def get_f(P, sys, melt, gamma):
        ...
        if run.FO2_buffer_SET:
            fo2 = gp.exp(cnvs.generate_fo2(sys, sys.run.FO2_buffer_START, sys.run.FO2_buffer, P))
        ...
        if run.NITROGEN_SET:
            n2_y = gamma[-1]
            fn2 = sl.n2_fugacity(run.NITROGEN_START, n2_y, float(fo2), P, name=run.N_MODEL)
        else:
            fn2 = 0          
    ...
    def find_p(P, sys, melt):
        ...
        P = gp.mpfr(re.sub('[\[\]]', '', np.array_str(P)))
    ...
    P_sat = gp.mpfr(re.sub('[\[\]]', '', np.array_str(P_sat)))
    sys.P = P_sat
    ...
    if run.FO2_buffer_SET:
        ...
        sys.FO2 = gp.log(cnvs.c2fo2(melt.Cm(), sys.T, sys.Ppa, sys.run.FO2_MODEL))
    ...
    if run.GAS_SYS == 'OH':
        ...
        mH2 = gp.mpfr(((1. - mO2) * H2O.Y) / (sys.K['K1'] * H2.Y * ((O2.Y * mO2 * sys.P)** 0.5) + H2O.Y))
        ...
        sys.atomicM['h'] = 2 * cnst.m['h'] * ((sys.WgT[0] / sum(mjMj)) * (mH2O + mH2) +
                                            sl.h2o_melt(mH2O, H2O, sys.P, name=run.H2O_MODEL) +
                                            sl.h2_melt(mH2, H2, sys.P, name=run.H2_MODEL))

        sys.atomicM['c'], sys.atomicM['s'], sys.atomicM['n'] = gp.mpfr('0'), gp.mpfr('0'), gp.mpfr('0')
        ...
        sys.atomicM['s'] = gp.mpfr('0')

        sys.atomicM['n'] = gp.mpfr('0')
        ...
        def quadratic():
            ...
            b = 1 + ((sys.K['K4'] * H2O.Y * mH2O * SO2.Y) / (sys.K['K5'] * H2S.Y * ((O2.Y * mO2) ** 1.5) * gp.sqrt(sys.P)))
            c = -(1 - mH2 - mO2 - mH2O)

            mSO2 = [(-b + gp.sqrt((b ** 2) - (4 * a * c))) / (2 * a), (-b - gp.sqrt((b ** 2) - (4 * a * c))) / (2 * a)]
            for item in mSO2:
                if item < 1:
                    return item
...
def satp_writeout(sys, melt, gas, P, values, gamma, mols, graph_sat=False):
    ...
    melt_h2 = sl.h2_melt(mH2, H2, P, name = sys.run.H2_MODEL, Y=h2y)*cnst.m['h2']
    ...
    if sys.run.GAS_SYS == "COHS" or sys.run.GAS_SYS == "COH" or sys.run.GAS_SYS == "COHSN":
        if sys.run.ATOMIC_MASS_SET == False and graph_sat == False:
            ...
            melt_co2 = gp.mpfr(sl.co2_melt((co2y*mCO2*P), CO2, (o2y*mO2*P), sys.T, P, melt, name=sys.run.C_MODEL)*cnst.m['co2'])
        ...
        mCO, mCO2, mCH4 = gp.mpfr('0'), gp.mpfr('0'), gp.mpfr('0')
        coy, co2y, ch4y = gp.mpfr('0'), gp.mpfr('0'), gp.mpfr('0')

    if sys.run.GAS_SYS == "COHS" or sys.run.GAS_SYS == "SOH" or sys.run.GAS_SYS == "COHSN":
        fo2 = (o2y * mO2 * P)
        melt.cm_dry, F = melt.iron_fraction(gp.log(fo2), ppa = P*1e5) # Uses fO2 to set the Fe2/Fe3 ratio and dry melt chemistry prior to needing sulfide capacity
        ...
        mS2, mSO2, mH2S = gp.mpfr('0'), gp.mpfr('0'), gp.mpfr('0')
        s2y, so2y, h2sy = gp.mpfr('0'), gp.mpfr('0'), gp.mpfr('0')

        fo2 = (o2y * mO2 * P)
        melt.cm_dry, F = melt.iron_fraction(gp.log(fo2), ppa = P*1e5)
    ...
    if sys.run.GAS_SYS == "COHSN":
        ...
        melt_n, mN2, n2y = gp.mpfr('0'), gp.mpfr('0'), gp.mpfr('0')
    ...
    return f"{P:.4f} \t {cnvs.generate_fo2_buffer(sys, (o2y * mO2 * P), P):+.3} \t {fo2:.3e} \t {F:.3} \t {rho_bulk:.3} \t {rho_melt:.3} \t {GvF * 100:.3g} \t {sys.WgT[0] * 100:.3g} \t {M:.3g} \t {gas.mH2O[0]:.g} \t {gas.mH2[0]:.g} \t {gas.mO2[0]:.g} \t {gas.mCO2[0]:.g} \t {gas.mCO[0]:.g} \t {gas.mCH4[0]:.g} \t {gas.mSO2[0]:.g} \t {gas.mH2S[0]:.g} \t {gas.mS2[0]:.g} \t {gas.mN2[0]:.g} \t {wts['H2O']:.g} \t {wts['H2']:.g} \t {wts['O2']:.g} \t {wts['CO2']:.g} \t {wts['CO']:.g} \t {wts['CH4']:.g} \t {wts['SO2']:.g} \t {wts['H2S']:.g} \t {wts['S2']:.g} \t {wts['N2']:.g} \t {melt_h2o*100:.g} \t {melt_h2*100:.g} \t {melt_co2*100:.g} \t {melt_co*100:.g} \t {melt_ch4*100:.g} \t {graphite*100:.g} \t {s2_melt*100:.g} \t {s6_melt*100:.g} \t {melt_s*100:.g} \t {melt_n*100:.g} \t {gas.mCO2[0]/gas.mCO[0]:.g} \t {gas.mCO2[0]/gas.mH2O[0]:.g} \t {gas.mCO2[0]/gas.mSO2[0]:.g} \t {gas.mH2S[0]/gas.mSO2[0]:.g} \t {(h2y * mH2 * P):.8g} \t {(h2oy * mH2O * P):.6g} \t {(co2y * mCO2 * P):.6g} \t {(coy * mCO * P):.6g} \t {(ch4y * mCH4 * P):.6g} \t {(so2y * mSO2 * P):.8g} \t {(h2sy * mH2S * P):.8g} \t {(s2y * mS2 * P):.8g} \t {(n2y * mN2 * P):.8g} \t {sys.atomicM['h']*1000000:.8g} \t {sys.atomicM['c']*1000000:.8g} \t {atomico*1000000:.8g} \t {cnvs.atomicM_calc(sys, melt, gas, 'o_tot', 0)*1000000:.8g} \t {sys.atomicM['s']*1000000:.8g} \t {sys.atomicM['n']*1000000:.8g} \n"
