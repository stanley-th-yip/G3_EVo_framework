...
class RunDef:
    ...
    def __init__(self):
        ...
        self.CALC_PRECISION = 113               # equivalent to quadruple precision
        gp.get_context().precision = self.CALC_PRECISION  # sets the global precision of mpfr values to 113 bits (33 dp)
        ...
        self.FORCE_CONSTANT_FO2 = False         # Forces the fO2 to stay constant throughout a decompression
        self.OCS = False
        self.S_SAT_WARN = True                    # Turns the check on sulphur saturation on/off for a system including S and Fe_system on.

        # model parameters
        self.T_START = gp.mpfr('1473.15')       # K
        self.P_START = gp.mpfr('3000')          # bar
        self.P_STOP = gp.mpfr('1')              # bar
        self.DP_MIN = gp.mpfr('1')              # Minimum pressure step
        self.DP_MAX = gp.mpfr('100')            # Maximum pressure step
        self.MASS = gp.mpfr('100')              # System mass, g
        self.WgT = gp.mpfr('0.001')             # Initial total gas FRACTION (not percent)
        self.LOSS_FRAC = gp.mpfr('0.99')        # The fraction of the gas phase lost at each step during open system degassing
        ...
        self.FO2_buffer = 'FMQ'
        self.FO2_buffer_START = gp.mpfr('0.0')  #delta buffer value
        self.FO2_SET = False
        self.FO2_START = gp.mpfr('0.0')         # bar
        self.FH2_SET = False
        self.FH2_START = gp.mpfr('0.24')        # bar
        self.FH2O_SET = False
        self.FH2O_START = gp.mpfr('1000')       # bar
        self.FCO2_SET = False
        self.FCO2_START = gp.mpfr('0.01')

        # set to run using total atomic mass fractions - ppm
        self.ATOMIC_MASS_SET = False
        self.ATOMIC_H = gp.mpfr('550.0')
        self.ATOMIC_C = gp.mpfr('200.0')
        self.ATOMIC_S = gp.mpfr('4000.0')
        self.ATOMIC_N = gp.mpfr('10.0')

        # initial wt frac volatile in magma (at given T and P, not total)
        self.WTH2O_SET = False
        self.WTH2O_START = gp.mpfr('0.03')      # initial melt FRACTION
        self.WTCO2_SET = False
        self.WTCO2_START = gp.mpfr('0.01')      # initial melt FRACTION
        self.SULFUR_SET = False
        self.SULFUR_START = gp.mpfr('0.001')    # initial melt FRACTION
        self.NITROGEN_SET = False
        self.NITROGEN_START = gp.mpfr('0.01')   # initial melt FRACTION
        self.GRAPHITE_SATURATED = False
        self.GRAPHITE_START = gp.mpfr('0')      # initial melt graphite content


    def param_set(self, params):

        # expect dictionary of key:value pairs
        # force type based on what's already been set (forces MPFR particularly)
        for par, val in params.items():
            if self.zTest_Params(par):
                if isinstance(getattr(self, par), type(gp.mpfr(0))):
                    val = str(val)
                    setattr(self, par, gp.mpfr(val)) 
                elif isinstance(getattr(self, par), bool):  # PL: Rest of this probs not necessary, yaml does this automatically.
                    setattr(self, par, bool(val))
                elif isinstance(getattr(self, par), int):
                    setattr(self, par, int(val))
                elif isinstance(getattr(self, par), str):
                    setattr(self, par, str(val))
        ...
        if self.COMPOSITION != 'basalt' and self.COMPOSITION != 'phonolite' and self.COMPOSITION != 'rhyolite':
            exit("Error: This composition is invalid. Please enter 'basalt', 'phonolite', or 'rhyolite'.")
...
class ThermoSystem:
    ...
    def norm_with_gas(self, melt, mols, gas):  # for iron equilibration.
        ...
        for ele in C_w:
            if ele is 'feo' or ele is 'fe2o3': # PL: This doesn't convert fe2o3 to feo at the moment, it probably should? Need to catch scenario where only fe2o3 is provided?
                cons += C_w[ele]/100
...
class Melt:
    ...
    def chem_init(self, eles, chem):
        ...
        if 'feo' in eles and 'fe2o3' in eles and self.sys.FO2 is None:
            self.sys.FO2 = gp.log(cnvs.c2fo2(self.Cm(), self.sys.T, self.sys.Ppa, self.run.FO2_MODEL))
    ...
    def formula_weight(self, fo2, P):
        ...
        mol_oxides = self.iron_fraction(gp.log(fo2), ppa = P*1e5)[0]
    ...
    def rho(self,T=None,P=None):
        ...
        if self.run.DENSITY_MODEL == "spera2000":
            return density.den_calc_spera2000(self.cm_density, T, P)
    ...
    def sulfide_sat(self, comp):
        ...
        if self.run.SCSS == 'liu2007':
            ...
            return gp.exp(11.35251 - 4454.6/self.sys.T - 0.0319*(self.sys.P/self.sys.T) + 0.71006*gp.log(MFM) - 1.98063*(MFM*mH2O) + 0.21867*gp.log(mH2O) + 0.36192*gp.log(mFeO))
    ...
    def melt_composition(self, gas, mols):
        ...
        self.h2.append(sl.h2_melt(gas.mH2[-1], H2, self.sys.P, name = self.sys.run.H2_MODEL)*cnst.m['h2'])
        ...
        if self.sys.run.GAS_SYS == "COHS" or self.sys.run.GAS_SYS == "COH" or self.sys.run.GAS_SYS == "COHSN":
            ...
            self.co2.append(gp.mpfr(sl.co2_melt((CO2.Y*gas.mCO2[-1]*self.sys.P), CO2, (O2.Y*gas.mO2[-1]*self.sys.P), self.sys.T, self.sys.P, self, name=self.sys.run.C_MODEL)*cnst.m['co2']))
            ...
            if self.sys.run.CH4_MODEL != 'None':
                ...
                self.ch4.append(gp.mpfr(0))

            if self.graphite_sat == True:
                self.graphite.append(self.graph_current*cnst.m['c'])
            else:
                self.graphite.append(gp.mpfr('0'))

        else:
            self.co2.append(gp.mpfr(0))
            self.co.append(gp.mpfr(0))
            self.ch4.append(gp.mpfr(0))
            self.graphite.append(gp.mpfr(0))
        ...
        if self.sys.run.GAS_SYS == "COHS" or self.sys.run.GAS_SYS == "SOH" or self.sys.run.GAS_SYS == "COHSN":
            ...
            self.cm_dry, F = self.iron_fraction(gp.log(fo2))
            self.F.append(F)
            ...
            self.sulfide.append(gp.mpfr(0))
            self.sulfate.append(gp.mpfr(0))            
            self.s.append(gp.mpfr(0))
...
class Gas:
    ...
    def get_WgT(self, melt, mols):
        ...
        if self.sys.run.GAS_SYS == 'OH':
            ...
            wgt = ((self.sys.atomicM['h']/(2*cnst.m['h']) - (sl.h2o_melt(self.mH2O[-1], H2O, self.sys.P, name=self.sys.run.H2O_MODEL) + sl.h2_melt(self.mH2[-1], H2, self.sys.P, name=self.sys.run.H2_MODEL)))/(self.mH2O[-1] + self.mH2[-1] + self.mH2S[-1])) * sum(mjMj)
    ...
    def get_vol_frac(self, melt):
        ...
        if self.sys.run.GAS_SYS == 'OH':
            ...
            empty_lists = {'CO2':gp.mpfr(0), 'CO':gp.mpfr(0), 'CH4':gp.mpfr(0), 'S2':gp.mpfr(0), 'SO2':gp.mpfr(0), 'H2S':gp.mpfr(0), 'N2':gp.mpfr(0)}
            M = cnvs.mean_mol_wt(H2O=self.mH2O[-1], O2=self.mO2[-1], H2=self.mH2[-1])
        elif self.sys.run.GAS_SYS == 'COH':
            empty_lists = {'S2':gp.mpfr(0), 'SO2':gp.mpfr(0), 'H2S':gp.mpfr(0), 'N2':gp.mpfr(0)}
            ...
            empty_lists = {'CO2':gp.mpfr(0), 'CO':gp.mpfr(0), 'CH4':gp.mpfr(0), 'N2':gp.mpfr(0)}
