...
def find_Y(P, T, species_list):
    ...
    for species in species_list:
        ...
        if species == "H2":
            # Shaw and Wones 1964
            logyH2 = gp.mpfr(P * gp.exp((-3.8402 * (T ** (1 / 8))) + 0.541) - P ** 2 * gp.exp(-0.1263 * (T ** 0.5) - 15.98) + 300 * gp.exp(-0.11901 * T - 5.941) * gp.exp(-(P / 300) - 1))
            ...
            lnf = (R*T*gp.log(1000.0*P_kb) + b*P_kb + (a/(b*gp.sqrt(T)))*(gp.log(R*T + b*P_kb) - gp.log(R*T + 2.0*b*P_kb)) + (2/3)*c*P_kb**1.5 + (d/2.0)*P_kb**2.0)/(R*T)
            
            co2_y = gp.exp(lnf)/(P_kb*1000.0)
            ...
            h2o_y = gp.exp(Z - 1 - gp.log(Z-MRKb) - MRKa*gp.log(1+(MRKb/Z)) + lnYvirial) # EQ. A.2
            ...
            A = 0.92854 + 0.43269e-1*Tr + -0.24671*Tr**-1 + 0.24999*Tr**-2 + -0.53182*Tr**-3 + -0.16461e-1*gp.log(Tr)
            B = 0.84866e-3 + -0.18379e-2*Tr + 0.66787e-1*Tr**-1 + -0.29427e-1*Tr**-2 + 0.29003e-1*Tr**-3 + 0.54808e-2*gp.log(Tr)
            C = -0.35456e-3 + 0.23316e-4*Tr + 0.94159e-3*Tr**-1 + -0.81653e-3*Tr**-2 + 0.23154e-3*Tr**-3 + 0.55542e-4*gp.log(Tr)
            D = 0.0
            integral = A*gp.log(Pr/P0r) + B*(Pr - P0r) + (C/2.0)*(Pr**2 - P0r**2) + (D/3.0)*(Pr**3 - P0r**3)
            
            so2_y = (gp.exp(integral) + 0)/P
            ...
            def q_geq(a, b, c, d, e, f, g, h, Tr):
                # when P>= 1000, except H2S
                return a + b*Tr + c*Tr**-1 + d*Tr**2 + e*Tr**-2 + f*Tr**3 + g*Tr**-3 + h*gp.log(Tr)
            
            def Z(a, b, c, d, pr, p0r):
                # returns the integral Z(p, t)/P, eq. 11
                return a*gp.log(pr/p0r) + b*(pr-p0r) + (c/2.0)*(pr**2 - p0r**2) + (d/3.0)*(pr**3 - p0r**3)
            ...
            def Z5000(pcrit, Tr):
                ...
                A = 1 + (-0.5917 / Tr**2)
                B = (0.09122 / Tr)
                C = (-0.0001416 / Tr**2) + (-0.000002835 * gp.log(Tr))
                D = 0.0
