...
def get_inputs(sys, run, melt, gas, mols):
    ...
    if run.FH2_SET == True:
        mH2 = gp.mpfr(run.FH2_START / (H2.Y * sys.P))
        gas.mH2.append(mH2)
        ...
        if sys.FO2 != None:  # if run.FO2 set was true, this is set in the readinenv file. If Fe ratio was set, it's created in the melt init from the melt chemistry
            mO2 = gp.mpfr(gp.exp(sys.FO2) / (O2.Y * sys.P))
            gas.mO2.append(mO2)
            ...
            mH2O = gp.mpfr(run.FH2O_START / (H2O.Y * sys.P))
            gas.mH2O.append(mH2O)

            mO2 = (((H2O.Y*mH2O*sys.P) / ((H2.Y*mH2*sys.P) * sys.K['K1'])) ** 2) / (O2.Y * sys.P)
            gas.mO2.append(mO2)
            sys.FO2 = gp.log(O2.Y * mO2 * sys.P)

        elif run.WTH2O_SET == True:
            mH2O = gp.mpfr(sl.h2o_fugacity(run.WTH2O_START, H2O, name=run.H2O_MODEL)/(H2O.Y*sys.P))
        ...
        mH2 = (H2O.Y*mH2O*sys.P) / (sys.K['K1']*H2.Y*sys.P*gp.exp(sys.FO2)**0.5)
    ...
    if not run.FCO2_SET and not run.WTCO2_SET and not run.SULFUR_SET and not run.GRAPHITE_SATURATED:
        ...
        mS2 = gp.mpfr(sl.S2_fugacity(run.SULFUR_START, (O2.Y*mO2*sys.P), sys.P, sys.T, melt, run, sulfidename=run.SULFIDE_CAPACITY, sulfatename=run.SULFATE_CAPACITY)/(S2.Y*sys.P))
        gas.mS2.append(mS2)
        
        return mS2, mH2O, mO2, mH2
    
    elif run.FCO2_SET:
        mCO2 = gp.mpfr(run.FCO2_START / (CO2.Y * sys.P))
        ...
        if run.C_MODEL == 'eguchi2018':
            fCO2 = sl.co2_fugacity(run.WTCO2_START, CO2, gp.exp(sys.FO2), sys.T, sys.P, melt, name=run.C_MODEL)
            if melt.graphite_sat == True:
                found = msgs.graphite_warn(melt)                
                if found == False:
                    mCO2 = gp.mpfr(sl.graphite_fco2(sys.T, sys.P, (O2.Y*mO2*sys.P))/(CO2.Y*sys.P))
                    gas.mCO2.append(mCO2)
                ...
                mCO2 = gp.mpfr(fCO2/(CO2.Y*sys.P))
                gas.mCO2.append(mCO2)

                melt.cm_dry = melt.iron_fraction(sys.FO2)[0]

        else:
            mCO2 = gp.mpfr(sl.co2_fugacity(run.WTCO2_START, CO2, gp.exp(sys.FO2), sys.T, sys.P, melt, name=run.C_MODEL)/(CO2.Y*sys.P))
            gas.mCO2.append(mCO2)
...
def oh(sys, run, gas, mols):
    H2O, O2, H2 = mols

    if run.FH2_SET == True:
        fH2 = run.FH2_START
        assert fH2 > 0, 'fH2 is negative!'
        mH2 = gp.mpfr(fH2 / (H2.Y * sys.P))
        gas.mH2.append(mH2)
        ...
        def quadratic():
            ...
            mO2 = [(-b + gp.sqrt((b ** 2) - (4 * a * c))) / (2 * a), (-b - gp.sqrt((b ** 2) - (4 * a * c))) / (2 * a)]
            for item in mO2:
        ...
        fO2 = gp.exp(sys.FO2)
        mO2 = gp.mpfr(fO2 / (O2.Y * sys.P))
        gas.mO2.append(mO2)

        mH2 = gp.mpfr(((1. - mO2) * H2O.Y) / (sys.K['K1'] * H2.Y * (fO2 ** 0.5) + H2O.Y))
        gas.mH2.append(mH2)
...
def soh(sys, run, melt, gas, mols):
    ...
    def quadratic():
        ...
        mSO2 = [(-b + sqrt((b ** 2) - (4 * a * c))) / (2 * a), (-b - sqrt((b ** 2) - (4 * a * c))) / (2 * a)]
        for item in mSO2:
            if item < 1:
                return item
...
def cohsn(sys, run, melt, gas, mols):
    ...
    if not run.SULFUR_SET:
        ...
        mN2 = gp.mpfr(sl.n2_fugacity(run.NITROGEN_START, N2.Y, (float(O2.Y) * float(mO2) * float(sys.P)), sys.P, name=run.N_MODEL)/(N2.Y*sys.P))
