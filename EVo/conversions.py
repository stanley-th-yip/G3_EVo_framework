...
def truncate(n, decimals=0):
    ...
    return gp.floor(n * multiplier) / multiplier
...
def generate_fo2(sys, dfo2, buffer, P):
    ...
    if buffer == 'FMQ':
        fo2 = gp.log(10 ** fmq2fo2(dfo2, sys.T, P, sys.run.FMQ_MODEL))  # ln fO2

    elif buffer == 'IW':
        fo2 = gp.log(10 ** iw2fo2(dfo2, sys.T, P, sys.run.FMQ_MODEL))
 
    elif buffer == 'NNO':
        fo2 = gp.log(10 ** nno2fo2(dfo2, sys.T, P, sys.run.FMQ_MODEL))
...
def generate_fo2_buffer(sys, fo2, P, buffer_choice = None):
    ...
    if buffer_choice is not None:
        ...
        return fo2_2fmq(gp.log10(fo2), sys.T, P, sys.run.FMQ_MODEL)
    
    if buffer == 'FMQ':
        return fo2_2fmq(gp.log10(fo2), sys.T, P, sys.run.FMQ_MODEL)

    elif buffer == 'IW':
        return fo2_2iw(gp.log10(fo2), sys.T, P, sys.run.FMQ_MODEL)
    
    elif buffer == 'NNO':
        return fo2_2nno(gp.log10(fo2), sys.T, P, sys.run.FMQ_MODEL)
