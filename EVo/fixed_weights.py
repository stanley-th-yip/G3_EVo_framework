...
def sat_pressure(run, sys, gas, melt, mols):
    ...
    def get_f(P, melt_h2o, melt_co2, melt_s, melt_n, sys, melt, gamma):
        ...
        if melt_n!= 0.0:
            n2_y = gamma[-1]
            fn2 = sl.n2_fugacity(melt_n, n2_y, float(fo2), P, name=run.N_MODEL)
        else:
            fn2 = 0          
    ...
    def fixed_weights_oh(guesses=[0.0]):
        ...
        return [(N * (mH2O + mH2) + sl.h2_melt(mH2, H2, P_sat, name=run.H2_MODEL, Y=h2_y) + sl.h2o_melt(mH2O, H2O, P_sat, name=run.H2O_MODEL, Y=h2o_y)) - (sys.atomicM['h']/(2*cnst.m['h']))]
    ...
    def fixed_weights_coh(guesses=[0.0, 0.0]):
        ...
        return [(N * (mH2O + mH2 + 2*mCH4) + sl.h2_melt(mH2, H2, P_sat, name=run.H2_MODEL, Y=h2_y) + sl.h2o_melt(mH2O, H2O, P_sat, name=run.H2O_MODEL, Y=h2o_y) + 2*sl.ch4_melt((ch4_y*mCH4*P_sat), P_sat, name = run.CH4_MODEL)) - (sys.atomicM['h']/(2*cnst.m['h'])),
    ...
    def fixed_weights_soh(guesses=[0.0, 0.0]):
        ...
        return [(N * (mH2O + mH2 + mH2S) + sl.h2_melt(mH2, H2, P_sat, name=run.H2_MODEL, Y=h2_y) + sl.h2o_melt(mH2O, H2O, P_sat, name=run.H2O_MODEL, Y=h2o_y)) - (sys.atomicM['h']/(2*cnst.m['h'])),
    ...
    def fixed_weights_cohs(guesses=[0.0, 0.0, 0.0]):
        ...
        return [(N * (mH2O + mH2 + mH2S + 2*mCH4) + sl.h2_melt(mH2, H2, P_sat, name=run.H2_MODEL, Y=h2_y) + sl.h2o_melt(mH2O, H2O, P_sat, name=run.H2O_MODEL, Y=h2o_y) + 2*sl.ch4_melt((ch4_y*mCH4*P_sat), P_sat, name = run.CH4_MODEL)) - (sys.atomicM['h']/(2*cnst.m['h'])),
    ...
    def fixed_weights_cohsn(guesses=[0.0, 0.0, 0.0, 0.0]):
        ...
        (N * (mH2O + mH2 + mH2S + 2*mCH4) + sl.h2_melt(mH2, H2, P_sat, name=run.H2_MODEL, Y=h2_y) + sl.h2o_melt(mH2O, H2O, P_sat, name=run.H2O_MODEL, Y=h2o_y) + 2*sl.ch4_melt((ch4_y*mCH4*P_sat), P_sat, name = run.CH4_MODEL)) - (sys.atomicM['h']/(2*cnst.m['h'])),
    ...
    if run.FO2_buffer_SET == True and cnvs.generate_fo2_buffer(sys, gp.exp(cnvs.generate_fo2(sys, run.FO2_buffer_START, run.FO2_buffer, 300)), 300, buffer_choice = 'IW') < -1.5:
        guess_h2o = float(((run.ATOMIC_H/1e6)/cnst.m['h'])*0.2*cnst.m['h2o'])
    else:
        guess_h2o = float((((run.ATOMIC_H/1e6)/cnst.m['h'])*cnst.m['h2o'])/2)
    
    if run.GAS_SYS == 'COH' or run.GAS_SYS == 'COHS' or run.GAS_SYS == 'COHSN':
        if run.C_MODEL == 'eguchi2018' and cnvs.generate_fo2_buffer(sys, gp.exp(cnvs.generate_fo2(sys, run.FO2_buffer_START, run.FO2_buffer, 300)), 300, buffer_choice = 'IW') < -1.5:
            guess_co2 = 1e-5
            melt.graphite_sat == True
        elif run.CO_MODEL != 'None' and run.CH4_MODEL != 'None':
            guess_co2 = float((((run.ATOMIC_C/1e6)/cnst.m['c'])*cnst.m['co2']*0.2))
        else:
            guess_co2 = float((((run.ATOMIC_C/1e6)/cnst.m['c'])*cnst.m['co2']))
    else:
        melt_co2 = 0
    
    if run.GAS_SYS == 'SOH' or run.GAS_SYS == 'COHS' or run.GAS_SYS == 'COHSN': 
        guess_s = float(run.ATOMIC_S/1e6)
    else:
        melt_s = 0
    
    if run.GAS_SYS == 'COHSN':
        guess_n = float(run.ATOMIC_N/1e6)
    else:
        melt_n = 0
    ...
    if run.GAS_SYS == 'OH':
        ...
        sys.atomicM['h'], sys.atomicM['c'], sys.atomicM['s'], sys.atomicM['n'] = gp.mpfr('0'), gp.mpfr('0'), gp.mpfr('0'), gp.mpfr('0')
        ...
        sys.atomicM['c'], sys.atomicM['n'] = gp.mpfr('0'), gp.mpfr('0')
